...
(function (Date, undefined) {
    ...
    Ansi_Up.prototype.escape_for_html = function (txt) {
      if(!txt){return '';} //stops null from printing
    ...
    // Ansi_Up.prototype.picafy = function (txt) {
    ...
    Ansi_Up.prototype.linkify = function (txt) {
      if(!txt){return '';} //stops null from printing
      return txt.replace(/(?:http:\/\/)?(?:www\.)?(?:youtube\.com|youtu\.be)\/(?:watch\?v=)?(.+)/g, '<br/><iframe width="420" height="345" src="http://www.youtube.com/embed/$1" frameborder="0" allowfullscreen></iframe>');
      return txt.replace(/(https?:\/\/[^\s]+)/gm, function(str) {
